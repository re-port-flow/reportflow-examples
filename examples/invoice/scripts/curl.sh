...
EX_DIR="$(cd "$(dirname "$0")/.." && pwd)"       # examples/invoice
REPO_ROOT="$(cd "$EX_DIR/../.." && pwd)"
[ -f "$REPO_ROOT/.env" ] && { set -a; . "$REPO_ROOT/.env"; set +a; }

: "${REPORTFLOW_API_KEY:?REPORTFLOW_API_KEY を .env に設定してください (ak_...)}"
DESIGN_ID="${INVOICE_DESIGN_ID:?テンプレート複製後の自分のデザインIDを INVOICE_DESIGN_ID に設定してください}"
VERSION="${INVOICE_DESIGN_VERSION:-1}"

# input.json = content.params。リクエストボディを組み立てて POST する。
BODY="$(jq -n \
  --arg designId "$DESIGN_ID" \
  --argjson version "$VERSION" \
  --arg fileName "invoice.pdf" \
