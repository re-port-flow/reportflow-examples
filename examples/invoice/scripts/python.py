#!/usr/bin/env python3
# 請求書 サンプル — 同期 PDF 生成 (POST /v1/file/sync/single)
...
ex_dir = Path(__file__).resolve().parent.parent          # examples/invoice
...
design_id = os.environ.get("INVOICE_DESIGN_ID")
if not design_id:
    sys.exit("テンプレート複製後の自分のデザインIDを INVOICE_DESIGN_ID に設定してください")
version = int(os.environ.get("INVOICE_DESIGN_VERSION", "1"))

params = json.loads((ex_dir / "input.json").read_text(encoding="utf-8"))
body = {"designId": design_id, "version": version,
        "content": {"fileName": "invoice.pdf", "params": params}}
