...
EX_DIR="$(cd "$(dirname "$0")/.." && pwd)"       # examples/quotation
REPO_ROOT="$(cd "$EX_DIR/../.." && pwd)"
[ -f "$REPO_ROOT/.env" ] && { set -a; . "$REPO_ROOT/.env"; set +a; }

: "${REPORTFLOW_API_KEY:?REPORTFLOW_API_KEY を .env に設定してください (ak_...)}"
DESIGN_ID="${QUOTATION_DESIGN_ID:?テンプレート複製後の自分のデザインIDを QUOTATION_DESIGN_ID に設定してください}"
VERSION="${QUOTATION_DESIGN_VERSION:-1}"
