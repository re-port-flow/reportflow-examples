...
const exDir = path.resolve(__dirname, '..');            // examples/worker-roster
...
const designId = process.env.WORKER_ROSTER_DESIGN_ID;
if (!designId) { console.error('テンプレート複製後の自分のデザインIDを WORKER_ROSTER_DESIGN_ID に設定してください'); process.exit(1); }
const version = parseInt(process.env.WORKER_ROSTER_DESIGN_VERSION || '1', 10);

const params = JSON.parse(fs.readFileSync(path.join(exDir, 'input.json'), 'utf8'));
const body = { designId, version, content: { fileName: 'worker-roster.pdf', params } };
