...
ex_dir = Path(__file__).resolve().parent.parent          # examples/nda
...
design_id = os.environ.get("NDA_DESIGN_ID")
if not design_id:
    sys.exit("テンプレート複製後の自分のデザインIDを NDA_DESIGN_ID に設定してください")
version = int(os.environ.get("NDA_DESIGN_VERSION", "1"))

params = json.loads((ex_dir / "input.json").read_text(encoding="utf-8"))
body = {"designId": design_id, "version": version,
        "content": {"fileName": "nda.pdf", "params": params}}
