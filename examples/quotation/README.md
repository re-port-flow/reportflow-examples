...
Re:port Flow の「見積もり」テンプレートから PDF を生成するサンプルです。見積書。小計→消費税→合計の集計、見積有効期限、備考欄の長文折返しを確認します。

- **検証観点**: 小計・消費税・合計 / 有効期限 / 備考の長文 / 日本語表示
- **想定ページ数**: 1
- **対応 API バージョン**: `v1`（`POST /v1/file/sync/single`）
- **解説ページ**: [/examples/quotation](https://doc.re-port-flow.com/examples/quotation)

## 手順

### 1. テンプレートを複製する

[テンプレートギャラリーの「見積もり」](https://templates.re-port-flow.com/templates/0eUPHHS3Iejd8Smr) を開き、「テンプレートを複製」から自分の Re:port Flow ワークスペースへ複製します。
...
QUOTATION_DESIGN_ID=（複製したデザインID）
QUOTATION_DESIGN_VERSION=1
```
...
または、リポジトリルートから: `./scripts/generate.sh quotation`
